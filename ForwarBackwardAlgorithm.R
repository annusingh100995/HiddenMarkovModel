...
FORWARD_BACKWARD_ALGORITHM = function(TranstionMatrix, SegmantationMatrix, ObsSeq){
  ...
  # Return statement for EM algorithm, where filtered probilities, backward probailities and the probabities dor local decoding will be required.
  #return(list(filtered_prob, b_new_3, m_new))
}

########################################################################################
########################################################################################
Sequence_plot = function(Hidden_sequence, Estimated_hidden_sequence){
  
  d = dist(Hidden_sequence,Estimated_hidden_sequence)
  cat("Local decoding:", "\n")
  cat("Number of different elements:", d, "\n")
  cat("Distance between sequences:", d/length(Hidden_sequence), "\n")
  
  op=par(mfrow=c(2,1))
  plot(ts(Hidden_sequence),main="Truth")
  plot(ts(Estimated_hidden_sequence),main="Forward_Backward")
  par(op)
...
Estimated_hidden_seq_3_states = FORWARD_BACKWARD_ALGORITHM(Transition_matrix, Segmentation_matrix, obs_seq)
